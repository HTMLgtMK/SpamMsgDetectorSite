...
class inference(object):
	...
	model_dir='model/' # model1
        ...
        self.vecmodel=word2vec.sentence2vec('sgns.weibo.bigram-char')

        self.inputdata=tf.placeholder(tf.float32,[1,None,300])
        self.inputpadding=tf.placeholder(tf.float32,[1,None])
        self.pos=tf.placeholder(tf.float32,[None,32])
        classifier=model(True,1)
        self.outlabel=tf.argmax(classifier(self.inputdata,self.inputpadding,self.pos),1)
